...
BASE_EXPORT int StringTokenize(const std::string &input, const std::string &delimitor,
                               std::list<std::string> &output);
BASE_EXPORT int StringTokenize(const std::wstring &input, const std::wstring &delimitor,
                               std::list<std::wstring> &output);
BASE_EXPORT size_t StringTokenizeWithBlank(const std::wstring &input,
                                           const std::wstring &delimitor,
                                           std::vector<std::wstring> &outputWVector);
// replace all 'find' with 'replace' in the string
BASE_EXPORT size_t StringReplaceAll(const std::string &find, const std::string &replace,
                                    std::string &output);
BASE_EXPORT size_t StringReplaceAll(const std::wstring &find, const std::wstring &replace,
                                    std::wstring &output);
...
// the functions will try to convert the characters in input stream until the end of the
...
BASE_EXPORT int AsciiToHex(unsigned char *pAscii, unsigned char *pHex, int nLen);
BASE_EXPORT void HexToAscii(BYTE *pHex, BYTE *pAscii, int nLen);
...
#endif
// the following functions are used to validate encoded streamings, such as utf-8, gbk,
// big5, etc.
...
BASE_EXPORT std::string Byte2Hex(char bArray[], int bArray_len);
} // namespace ckbase
