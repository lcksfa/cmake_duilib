...
template <typename TSingleton, bool release_atexitmanager = true> class Singleton
{
...
#define SingletonHideConstructor(TSingleton)                                             \
    ...
    inline friend                                                                        \
        typename std::enable_if<!std::is_array<_Ty>::value, std::unique_ptr<_Ty>>::type  \
    ...
    static const TSingletonPtr &GetInstance()
    {
        ...
        std::call_once(oc, [&] {
            ...
            if (release_atexitmanager) {
                OnExitManager::RegisterCallback(
                    [&](void *) {
