...
struct MessageLoopThreadTlsData {
    MessageLoopThread *self; // A pointer pointed to the thread itself
    bool quit_properly;      // the thread quit properly
    int managed;             // reference counter, the framework thread is managed by
                             // MessageLoopThreadManager
    int managed_thread_id; // if a thread is managered, this will be the manager id of it
                           // (may not be a thread id)
    void *custom_data;     // reserved for the derived classes of MessageLoopThread
};
...
//
//  (1) MessageLoopThread::CleanUp()
//  (2) MessageLoop::~MessageLoop
//  (3.b)    MessageLoop::DestructionObserver::WillDestroyCurrentMessageLoop
class BASE_EXPORT MessageLoopThread : public virtual ckbase::SupportWeakCallback,
                                      public ckbase::Thread
{
    friend class ThreadMap;
    friend class MessageLoopThreadManager;
...
public:
    ...
    explicit MessageLoopThread(const char *name);
    ...
    // Do be sure that the current thread is a crt thread (created by _beginthreadex,
    // etc.) if OS = Windows
#if defined(OS_WIN)
    ...
    static MessageLoopThread *current();

    // Get the managed thread id of current thread
    // return -1 means current thread is not managed by MessageLoopThreadManager
...
protected:
    ...
    // Initialize the tls data, must be called before any tls associated calls on current
    // thread
    static void InitTlsData(MessageLoopThread *self);
    ...
    static MessageLoopThreadTlsData *GetTlsData();
