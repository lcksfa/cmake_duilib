// This file try to implement a cross flatform message loop proxy,
...
// MessageLoop.
class BASE_EXPORT MessageLoopProxy : public ckbase::SupportWeakCallback
{
public:
    ...
    bool PostTaskAndReply(const std::function<T1> &task, const std::function<T2> &reply)
    {
        ...
        if (!PostTask(ckbase::Bind(&PostTaskAndReplyRelay<T1, T2>::Run, relay))) {
            delete relay;
            return false;
        }
...
private:
    ...
    // considered preferable to having a thread-safety violations caused by
    ...
    template <typename T1, typename T2>
    class PostTaskAndReplyRelay : public ckbase::SupportWeakCallback
    {
    public:
        ...
        {
            auto ret = std_task_();
            origin_loop_->PostTask(ckbase::Bind(
                &PostTaskAndReplyRelay::RunReplyAndSelfDestructWithParam<decltype(ret)>,
                this, ret));
        }
    ...
    private:
        ...
        template <typename InernalT> void RunReplyAndSelfDestructWithParam(InernalT ret)
        {
            assert(origin_loop_->BelongsToCurrentThread());
...
template <> void MessageLoopProxy::PostTaskAndReplyRelay<void(), void()>::Run()
{
    std_task_();
    origin_loop_->PostTask(
        ckbase::Bind(&PostTaskAndReplyRelay::RunReplyAndSelfDestruct, this));
}
