...
public:
    ...
    {
        return block_ * allocator::requested_size;
    }
...
template <typename BlockAllocator, unsigned MaxBlocks>
inline bool
BlockBuffer<BlockAllocator, MaxBlocks>::increase_capacity(size_t increase_size)
{
    if (increase_size == 0)
        return true;
    ...
    // if (increase_size > maxsize() - free) throw std::out_of_range("BlockBuffer out of
    // range");
