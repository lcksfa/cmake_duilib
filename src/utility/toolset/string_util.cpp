                         ...
                         std::basic_string<CharType> &output)
{
     ...
     * we use two passes to finish the task in the case that replace.size() is greater
     * find.size()
     */
...
size_t StringReplaceAll(std::string &strSrc, const std::string &old_value,
                        const std::string &new_value)
{
    return StringReplaceAllT<char>(old_value, new_value, strSrc);
}

size_t StringReplaceAll(std::wstring &strSrc, const std::wstring &old_value,
                        const std::wstring &new_value)
{
    return StringReplaceAllT<wchar_t>(old_value, new_value, strSrc);
}

void HexToAscii(BYTE *pHex, BYTE *pAscii, int nLen)
{
    BYTE Nibble[2];
...
int AsciiToHex(unsigned char *pAscii, unsigned char *pHex, int nLen)
{
    int nHexLen;
    int m = 0;
    int i = 0, j = 0;
...
template <typename CharType> void StringTrimLeftT(std::basic_string<CharType> &output)
{
    size_t check = 0;
...
template <typename CharType> void StringTrimRightT(std::basic_string<CharType> &output)
{
    size_t length = output.length();
    const CharType *src = output.data();
...
size_t StringTokenizeWithBlank(const std::wstring &input, const std::wstring &delimitor,
                               std::vector<std::wstring> &outputWVector)
{
    std::wstring strTmp = input + delimitor;
    int len = delimitor.length();
    int nSatrt = 0;
    int nPos;
    while (true) {
        nPos = strTmp.find_first_of(delimitor, nSatrt);
        if (-1 == nPos)
            break;
        outputWVector.push_back(strTmp.substr(nSatrt, nPos - nSatrt));
        nSatrt = nPos + len;
    }
    return outputWVector.size();
}
...
std::string Byte2Hex(char bArray[], int bArray_len)
{
    char *szHex = new char[bArray_len * 2 + 1];
    std::string strHex;
    int nIndex = 0;
    for (int i = 0; i < bArray_len; i++) {
        unsigned char cHigh = bArray[i];
        int high = cHigh / 16, low = cHigh % 16;
        szHex[nIndex] = (high <= 9 && high >= 0) ? ('0' + high) : ('A' + high - 10);
        szHex[nIndex + 1] = (low <= 9 && low >= 0) ? ('0' + low) : ('A' + low - 10);
        nIndex += 2;
    }
    szHex[bArray_len * 2] = '\0';
    strHex = szHex;
    delete szHex;
    szHex = NULL;
    return strHex;
