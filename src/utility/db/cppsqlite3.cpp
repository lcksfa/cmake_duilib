...
CppSQLite3Exception::CppSQLite3Exception(const int nErrCode, const char *szErrMess,
...
void CppSQLite3Table::setRow(int nRow)
{
    ...
    if (nRow < 0 || nRow > mnRows - 1) {
        throw CppSQLite3Exception(CPPSQLITE_ERROR, "Invalid row index requested",
                                  DONT_DELETE_MSG);
    }
...
void CppSQLite3Statement::bind(int nParam, const unsigned char *blobValue, int nLen)
{
    checkVM();
    int nRes =
        sqlite3_bind_blob(mpVM, nParam, (const void *) blobValue, nLen, SQLITE_TRANSIENT);
...
int CppSQLite3DB::setKey(const char *key, unsigned int nlen)
{
    int nRet = -1;
    if (mpDB) {
        // FIXME: 暂时不支持密码设置
        // nRet = sqlite3_key(mpDB, key, nlen);
    }
    return nRet;
}
