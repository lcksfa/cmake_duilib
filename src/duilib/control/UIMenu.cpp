...
namespace ui {
	...
	void CMenuElementUI::DoEvent(TEventUI& event)
	{
		if( event.Type == UIEVENT_MOUSEENTER )
		{
			...
			//2020/3/17 wangliang 新增菜单WM_MENUMOUSEENTER消息响应
			...
			if (IsWindow(GetManager()->GetPaintWindow()) && !isClosing) {
				if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
				{
					...
					if (!PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUMOUSEENTER, (WPARAM)pMenuCmd, (LPARAM)this))
					{
						delete pMenuCmd;
						pMenuCmd = NULL;
					}
		...
		if (event.Type == UIEVENT_MOUSELEAVE) {
			...
			if (!hasSubMenu) {
				...
				//2020/3/17 wangliang 新增菜单WM_MENUMOUSELEAVE消息响应
				bool isClosing = false;
				CMenuUI* menuUI = static_cast<CMenuUI*>(GetManager()->GetRoot());
				isClosing = (menuUI->m_pWindow->isClosing);
				if (IsWindow(GetManager()->GetPaintWindow()) && !isClosing) {
					if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
					{
						...
						lstrcpy(pMenuCmd->szText, GetText().GetData());
						pMenuCmd->bChecked = GetChecked();
						if (!PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUMOUSELEAVE, (WPARAM)pMenuCmd, (LPARAM)this))
						{
							delete pMenuCmd;
							pMenuCmd = NULL;
						}
