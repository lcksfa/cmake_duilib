...
void CButtonUI::PaintText(HDC hDC)
{
    ...
    if (m_bShowHtml)
        CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText, clrColor, NULL, NULL,
                                    nLinks, iFont, m_uTextStyle);
    else
        CRenderEngine::DrawText(hDC, m_pManager, rc, sText, clrColor, iFont,
                                m_uTextStyle);
}
...
void CButtonUI::PaintStatusImage(HDC hDC)
{
    if (!m_sStateImage.IsEmpty() && m_nStateCount > 0) {
        ...
        const TImageInfo *pImage = m_pManager->GetImageEx(info.sImageName, info.sResType,
                                                          info.dwMask, info.bHSL);
            ...
            if (szImage.cx > 0 && szImage.cy > 0) {
                ...
                if (m_nStateCount > 0) {
                    ...
                    m_sNormalImage.Format(
                        ...
                        _T("source='%d,%d,%d,%d'"),
                        info.sImageName.GetData(), info.sResType.GetData(),
                        info.rcDest.left, info.rcDest.top, info.rcDest.right,
                        info.rcDest.bottom, iLeft, iTop, iRight, iBottom);
                }
                if (m_nStateCount > 1) {
                    ...
                    int iTop = rcSrc.top;
                    int iBottom = iTop + szStatus.cy;
                    m_sHotImage.Format(_T("res='%s' restype='%s' dest='%d,%d,%d,%d' ")
                                       _T("source='%d,%d,%d,%d'"),
                                       info.sImageName.GetData(), info.sResType.GetData(),
                                       info.rcDest.left, info.rcDest.top,
                                       info.rcDest.right, info.rcDest.bottom, iLeft, iTop,
                                       iRight, iBottom);
