    ...
    if (IsWindows8Point1OrGreater()) {
        ...
        if (hModule != NULL) {
            ...
            if (GetDpiForMonitor != NULL &&
                ...
                MessageBox(NULL, _T("GetDpiForMonitor failed"), _T("Notification"),
                           MB_OK);
                return 96;
            }
        }
    ...
    if (IsWindows8Point1OrGreater()) {
        ...
        if (hModule != NULL) {
            LPGetProcessDpiAwareness GetProcessDpiAwareness =
                (LPGetProcessDpiAwareness) GetProcAddress(hModule,
...
BOOL CDPI::SetDPIAwareness(PROCESS_DPI_AWARENESS Awareness)
{
    ...
    if (IsWindowsVistaOrGreater()) {
        ...
        if (set_process_dpi_awareness_func) {
            HRESULT hr = set_process_dpi_awareness_func(Awareness);
            if (S_OK == hr) {
                return true;
            }
            else if (hr == E_ACCESSDENIED) {
                //"Access denied error from SetProcessDpiAwareness. Function called twice,
                // or manifest was used.";
            }
        }
    }
    else {
