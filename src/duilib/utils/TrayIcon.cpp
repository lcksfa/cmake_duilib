...
void CTrayIcon::CreateTrayIcon(HWND _RecvHwnd, UINT _IconIDResource, LPCTSTR _ToolTipText,
                               UINT _Message)
{
...
bool CTrayIcon::SetInfoText(LPCTSTR _ToolInfoText, LPCTSTR _ToolInfoTitleText)
{
    if (!m_bEnabled)
        return FALSE;

    if (_ToolInfoText)
        _tcscpy(m_trayData.szInfo, _ToolInfoText);
    if (_ToolInfoTitleText)
        _tcscpy(m_trayData.szInfoTitle, _ToolInfoTitleText);

    m_trayData.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP | NIF_INFO;
    return Shell_NotifyIcon(NIM_MODIFY, &m_trayData) == TRUE;
}

ui::CDuiString CTrayIcon::GetTooltipText() const
{
