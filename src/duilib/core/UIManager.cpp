...
/////////////////////////////////////////////////////////////////////////////////////
typedef BOOL(__stdcall *PFUNCUPDATELAYEREDWINDOW)(HWND, HDC, POINT *, SIZE *, HDC,
    ...
    , m_iHoverTime(400UL)
{
    ...
    m_ResInfo.m_dwDefaultLinkHoverFontColor =
...
void CPaintManagerUI::SetInitSize(int cx, int cy)
{
    ...
    if (m_pRoot == NULL && m_hWndPaint != NULL) {
        ::SetWindowPos(m_hWndPaint, NULL, 0, 0, cx, cy,
...
void CPaintManagerUI::SetLayered(bool bLayered)
{
    if (m_hWndPaint != NULL && bLayered != m_bLayered) {
        ...
        if (g_fUpdateLayeredWindow == NULL) {
            HMODULE hUser32 = ::GetModuleHandle(_T("User32.dll"));
            if (hUser32) {
                g_fUpdateLayeredWindow = (PFUNCUPDATELAYEREDWINDOW)::GetProcAddress(
                    hUser32, "UpdateLayeredWindow");
                if (g_fUpdateLayeredWindow == NULL)
                    return;
            }
        }
...
bool CPaintManagerUI::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam,
                                        LRESULT & /*lRes*/)
{
    ...
    case WM_KEYDOWN: {
        ...
        if (wParam == VK_TAB) {
            ...
            if (m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled() &&
                _tcsstr(m_pFocus->GetClass(), _T("WkeWebkitUI")) != NULL) {
                return false;
            }
    ...
    case WM_SYSCHAR: {
        ...
        CControlUI *pControl =
            m_pRoot->FindControl(__FindControlFromShortcut, &fs,
                                     ...
                                     LRESULT &lRes)
{
        ...
        if (m_bUpdateNeeded) {
            ...
            if (!::IsRectEmpty(&rcClient) && !::IsIconic(m_hWndPaint)) {
                if (m_pRoot->IsUpdateNeeded()) {
                    ...
                    m_pRoot->FindControl(__FindControlsFromUpdate, NULL,
            ...
            RECT rcRoot = rcClient;
            if (m_pOffscreenBits)
                ::ZeroMemory(m_pOffscreenBits, (rcRoot.right - rcRoot.left) *
        ...
        if (m_bOffscreenPaint) {
            ...
            if (m_bLayered) {
                for (LONG y = rcClient.bottom - rcPaint.bottom;
                     y < rcClient.bottom - rcPaint.top; ++y) {
            ...
            if (m_bLayered) {
                for (int i = 0; i < m_aNativeWindow.GetSize();) {
                    ...
                    HBITMAP hChildBitmap = CRenderEngine::CreateARGB32Bitmap(
                        hChildMemDC, rcChildWnd.right - rcChildWnd.left,
                        rcChildWnd.bottom - rcChildWnd.top, (BYTE **) &pChildBitmapBits);
                    ...
                    ::SendMessage(hChildWnd, WM_PRINT, (WPARAM) hChildMemDC,
                                  (LPARAM)(PRF_CHECKVISIBLE | PRF_CHILDREN | PRF_CLIENT |
                                           PRF_OWNED));
                    ...
                    for (LONG y = 0; y < rcChildWnd.bottom - rcChildWnd.top; y++) {
                        for (LONG x = 0; x < rcChildWnd.right - rcChildWnd.left; x++) {
                            pChildBitmapBit = pChildBitmapBits +
                                              y * (rcChildWnd.right - rcChildWnd.left) +
                                              x;
                            if (*pChildBitmapBit != 0x00000000)
                                *pChildBitmapBit |= 0xff000000;
                        }
                    }
                    ::BitBlt(m_hDcOffscreen, rcChildWnd.left, rcChildWnd.top,
                             rcChildWnd.right - rcChildWnd.left,
                             rcChildWnd.bottom - rcChildWnd.top, hChildMemDC, 0, 0,
                             SRCCOPY);
            ...
            if (m_bLayered) {
                    ...
                    if (!m_diLayered.sDrawString.IsEmpty()) {
                        if (m_hbmpBackground == NULL) {
                            ...
                            m_hbmpBackground = CRenderEngine::CreateARGB32Bitmap(
                                m_hDcPaint, dwWidth, dwHeight,
                                (BYTE **) &m_pBackgroundBits);
                            ::ZeroMemory(m_pBackgroundBits, dwWidth * dwHeight * 4);
                            ::SelectObject(m_hDcBackground, m_hbmpBackground);
                            CRenderClip clip;
                            CRenderClip::GenerateClip(m_hDcBackground, rcLayeredClient,
                                                      clip);
                            CRenderEngine::DrawImageInfo(m_hDcBackground, this,
                                                         rcLayeredClient, rcLayeredClient,
                                                         &m_diLayered);
                        }
                    ...
                    for (LONG y = rcClient.bottom - rcPaint.bottom;
                        ...
                        for (LONG x = rcPaint.left; x < rcPaint.right; ++x) {
                            ...
                            if ((m_pOffscreenBits[i + 3] == 0) &&
                                (m_pOffscreenBits[i + 0] != 0 ||
                                 m_pOffscreenBits[i + 1] != 0 ||
                ...
                POINT ptSrc = {0, 0};
                g_fUpdateLayeredWindow(m_hWndPaint, m_hDcPaint, &ptPos, &sizeWnd,
                                       m_hDcOffscreen, &ptSrc, 0, &bf, ULW_ALPHA);
            }
            else {
                ::BitBlt(m_hDcPaint, rcPaint.left, rcPaint.top,
            ...
            if (m_bShowUpdateRect && !m_bLayered) {
                ...
                ::Rectangle(m_hDcPaint, rcPaint.left, rcPaint.top, rcPaint.right,
                            rcPaint.bottom);
    ...
    case WM_PRINTCLIENT: {
        ...
        if ((lParam & PRF_CHILDREN) != 0) {
            ...
            while (hWndChild != NULL) {
                ...
                ::MapWindowPoints(HWND_DESKTOP, m_hWndPaint,
                                  reinterpret_cast<LPPOINT>(&rcPos), 2);
    ...
    case WM_GETMINMAXINFO: {
        ...
        ::GetMonitorInfo(::MonitorFromWindow(m_hWndPaint, MONITOR_DEFAULTTOPRIMARY),
                         &Monitor);
    ...
    case WM_MOUSEHOVER: {
        ...
        if (m_hwndTooltip == NULL) {
            m_hwndTooltip = ::CreateWindowEx(
                ...
                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_hWndPaint,
                NULL, m_hInstance, NULL);
            ::SendMessage(m_hwndTooltip, TTM_ADDTOOL, 0, (LPARAM) &m_ToolTip);
            ::SendMessage(m_hwndTooltip, TTM_SETMAXTIPWIDTH, 0,
                          pHover->GetToolTipWidth());
        }
    ...
    case WM_MOUSEMOVE: {
        ...
        if (!m_bMouseTracking) {
            ...
            tme.dwHoverTime = m_hwndTooltip == NULL
                                  ? m_iHoverTime
                                  : (DWORD)::SendMessage(m_hwndTooltip, TTM_GETDELAYTIME,
                                                         TTDT_INITIAL, 0L);
        ...
        if (bNeedDrag && m_bDragMode && wParam == MK_LBUTTON) {
            ...
            HBITMAP hBitmap =
                (HBITMAP) OleDuplicateData(m_hDragBitmap, fmtetc.cfFormat, NULL);
            ...
            dragSrcHelper.InitializeFromBitmap(hBitmap, ptDrag, rc,
                                               pdobj); // will own the bmp
            DWORD dwEffect;
            HRESULT hr =
        ...
        if (!IsCaptured()) {
            ...
            if (pNewHover != m_pEventHover && m_pEventHover != NULL) {
                ...
                m_pEventHover = NULL;
                if (m_hwndTooltip != NULL)
                    ::SendMessage(m_hwndTooltip, TTM_TRACKACTIVATE, FALSE,
                                  (LPARAM) &m_ToolTip);
            }
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (pControl->IsDragEnabled()) {
            ...
            m_hDragBitmap =
                CRenderEngine::GenerateBitmap(this, pControl, pControl->GetPos());
        }
    ...
    case WM_MOUSEWHEEL: {
        ...
        ::SendMessage(m_hWndPaint, WM_MOUSEMOVE, 0,
...
void CPaintManagerUI::Term()
{
    ...
    for (int i = 0; i < m_SharedResInfo.m_ImageHash.GetSize(); i++) {
        if (LPCTSTR key = m_SharedResInfo.m_ImageHash.GetAt(i)) {
            data =
                static_cast<TImageInfo *>(m_SharedResInfo.m_ImageHash.Find(key, false));
            if (data) {
    ...
    for (int i = 0; i < m_SharedResInfo.m_CustomFonts.GetSize(); i++) {
        if (LPCTSTR key = m_SharedResInfo.m_CustomFonts.GetAt(i)) {
            pFontInfo =
                static_cast<TFontInfo *>(m_SharedResInfo.m_CustomFonts.Find(key, false));
            if (pFontInfo) {
    ...
    for (int i = 0; i < m_SharedResInfo.m_StyleHash.GetSize(); i++) {
        if (LPCTSTR key = m_SharedResInfo.m_StyleHash.GetAt(i)) {
            pStyle =
                static_cast<CDuiString *>(m_SharedResInfo.m_StyleHash.Find(key, false));
            if (pStyle) {
                delete pStyle;
                pStyle = NULL;
            }
    ...
    for (int i = 0; i < m_SharedResInfo.m_AttrHash.GetSize(); i++) {
        if (LPCTSTR key = m_SharedResInfo.m_AttrHash.GetAt(i)) {
            pAttr =
                static_cast<CDuiString *>(m_SharedResInfo.m_AttrHash.Find(key, false));
            if (pAttr) {
                delete pAttr;
                pAttr = NULL;
            }
...
CDPI *ui::CPaintManagerUI::GetDPIObj()
{
    if (m_pDPI == NULL) {
        m_pDPI = new CDPI;
    }
...
void ui::CPaintManagerUI::SetAdaptDPI()
{
    GetDPIObj()->SetDPIAwareness(PROCESS_PER_MONITOR_DPI_AWARE);
    SetDPI(GetDPIObj()->GetMainMonitorDPI());
}

void ui::CPaintManagerUI::SetDPI(int iDPI)
{
    ...
    SetWindowPos(GetPaintWindow(), NULL, prcNewWindow->left, prcNewWindow->top,
...
void ui::CPaintManagerUI::SetAllDPI(int iDPI)
{
    for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
        CPaintManagerUI *pManager = static_cast<CPaintManagerUI *>(m_aPreMessages[i]);
...
void ui::CPaintManagerUI::ResetDPIAssets()
{
    RemoveAllDrawInfos();
    RemoveAllImages();
    ;
    ...
    for (int it = 0; it < m_ResInfo.m_CustomFonts.GetSize(); it++) {
        TFontInfo *pFontInfo = static_cast<TFontInfo *>(
            m_ResInfo.m_CustomFonts.Find(m_ResInfo.m_CustomFonts[it]));
...
void ui::CPaintManagerUI::RebuildFont(TFontInfo *pFontInfo)
{
    ::DeleteObject(pFontInfo->hFont);
    LOGFONT lf = {0};
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
...
bool CPaintManagerUI::SetNextTabControl(bool bForward)
{
    ...
    if (pControl == NULL) {
        if (bForward) {
            ...
            pControl =
                m_pRoot->FindControl(__FindControlFromTab, &info2,
                                     UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
        }
        else {
...
void CPaintManagerUI::SendNotify(CControlUI *pControl, LPCTSTR pstrMessage,
...
{
    if (bShared) {
        if (m_ResInfo.m_dwDefaultDisabledColor ==
            m_SharedResInfo.m_dwDefaultDisabledColor)
...
{
    if (bShared) {
        if (m_ResInfo.m_dwDefaultLinkFontColor ==
            m_SharedResInfo.m_dwDefaultLinkFontColor)
...
{
    if (bShared) {
        if (m_ResInfo.m_dwDefaultSelectedBkColor ==
            m_SharedResInfo.m_dwDefaultSelectedBkColor)
    ...
    if (m_ResInfo.m_DefaultFontInfo.sFontName.IsEmpty()) {
        if (m_SharedResInfo.m_DefaultFontInfo.tm.tmHeight == 0) {
            HFONT hOldFont = (HFONT)::SelectObject(
                m_hDcPaint, m_SharedResInfo.m_DefaultFontInfo.hFont);
        ...
        if (m_ResInfo.m_DefaultFontInfo.tm.tmHeight == 0) {
            HFONT hOldFont =
                (HFONT)::SelectObject(m_hDcPaint, m_ResInfo.m_DefaultFontInfo.hFont);
...
void CPaintManagerUI::SetDefaultFont(LPCTSTR pStrFontName, int nSize, bool bBold,
                                     bool bUnderline, bool bItalic, bool bShared)
{
    LOGFONT lf = {0};
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
    ...
    if (bShared) {
        ...
        ::ZeroMemory(&m_ResInfo.m_DefaultFontInfo.tm,
                     sizeof(m_ResInfo.m_DefaultFontInfo.tm));
        if (m_hDcPaint) {
            HFONT hOldFont = (HFONT)::SelectObject(m_hDcPaint, hFont);
            ::GetTextMetrics(m_hDcPaint, &m_ResInfo.m_DefaultFontInfo.tm);
            ::SelectObject(m_hDcPaint, hOldFont);
...
HFONT CPaintManagerUI::AddFont(int id, LPCTSTR pStrFontName, int nSize, bool bBold,
                               bool bUnderline, bool bItalic, bool bShared)
{
    LOGFONT lf = {0};
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
    ...
    if (bShared || m_bForceUseSharedRes) {
        ...
        TFontInfo *pOldFontInfo =
            static_cast<TFontInfo *>(m_ResInfo.m_CustomFonts.Find(idBuffer));
        if (pOldFontInfo) {
            ::DeleteObject(pOldFontInfo->hFont);
            delete pOldFontInfo;
...
void CPaintManagerUI::AddFontArray(LPCTSTR pstrPath)
{
    ...
    do {
        ...
        if (CPaintManagerUI::GetResourceZip().IsEmpty()) {
            sFile += pstrPath;
            HANDLE hFile = ::CreateFile(sFile.GetData(), GENERIC_READ, FILE_SHARE_READ,
    ...
    while (!pData) {
        ...
        HANDLE hFile = ::CreateFile(pstrPath, GENERIC_READ, FILE_SHARE_READ, NULL,
...
HFONT CPaintManagerUI::GetFont(LPCTSTR pStrFontName, int nSize, bool bBold,
                               bool bUnderline, bool bItalic)
{
...
int CPaintManagerUI::GetFontIndex(HFONT hFont, bool bShared)
{
    ...
    if (bShared) {
        for (int i = 0; i < m_SharedResInfo.m_CustomFonts.GetSize(); i++) {
            if (LPCTSTR key = m_SharedResInfo.m_CustomFonts.GetAt(i)) {
                pFontInfo =
                    static_cast<TFontInfo *>(m_SharedResInfo.m_CustomFonts.Find(key));
...
int CPaintManagerUI::GetFontIndex(LPCTSTR pStrFontName, int nSize, bool bBold,
                                  bool bUnderline, bool bItalic, bool bShared)
{
    TFontInfo *pFontInfo = NULL;
    if (bShared) {
...
void CPaintManagerUI::RemoveAllFonts(bool bShared)
{
    ...
    if (bShared) {
        for (int i = 0; i < m_SharedResInfo.m_CustomFonts.GetSize(); i++) {
            if (LPCTSTR key = m_SharedResInfo.m_CustomFonts.GetAt(i)) {
                pFontInfo = static_cast<TFontInfo *>(
                    m_SharedResInfo.m_CustomFonts.Find(key, false));
                if (pFontInfo) {
                    ::DeleteObject(pFontInfo->hFont);
                    delete pFontInfo;
                }
        ...
        for (int i = 0; i < m_ResInfo.m_CustomFonts.GetSize(); i++) {
            if (LPCTSTR key = m_ResInfo.m_CustomFonts.GetAt(i)) {
                pFontInfo =
                    static_cast<TFontInfo *>(m_ResInfo.m_CustomFonts.Find(key, false));
                if (pFontInfo) {
                    ::DeleteObject(pFontInfo->hFont);
                    delete pFontInfo;
                }
...
const TImageInfo *CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask,
                                              bool bUseHSL, HINSTANCE instance)
{
...
const TImageInfo *CPaintManagerUI::AddImage(LPCTSTR bitmap, HBITMAP hBitmap, int iWidth,
                                            int iHeight, bool bAlpha, bool bShared)
{
...
void CPaintManagerUI::RemoveAllImages(bool bShared)
{
    if (bShared) {
        ...
        for (int i = 0; i < m_SharedResInfo.m_ImageHash.GetSize(); i++) {
            if (LPCTSTR key = m_SharedResInfo.m_ImageHash.GetAt(i)) {
                data = static_cast<TImageInfo *>(
                    m_SharedResInfo.m_ImageHash.Find(key, false));
                if (data) {
                    CRenderEngine::FreeImage(data);
                }
            }
...
void CPaintManagerUI::RemoveAllDrawInfos()
{
    ...
    for (int i = 0; i < m_ResInfo.m_DrawInfoHash.GetSize(); i++) {
        ...
        if (key != NULL) {
            pDrawInfo =
                static_cast<TDrawInfo *>(m_ResInfo.m_DrawInfoHash.Find(key, false));
            if (pDrawInfo) {
                delete pDrawInfo;
                pDrawInfo = NULL;
            }
...
void CPaintManagerUI::AddDefaultAttributeList(LPCTSTR pStrControlName,
                                              LPCTSTR pStrControlAttrList, bool bShared)
{
    if (bShared || m_bForceUseSharedRes) {
...
void CPaintManagerUI::RemoveAllDefaultAttributeList(bool bShared)
{
    if (bShared) {
        ...
        for (int i = 0; i < m_SharedResInfo.m_AttrHash.GetSize(); i++) {
            if (LPCTSTR key = m_SharedResInfo.m_AttrHash.GetAt(i)) {
                pDefaultAttr =
                    static_cast<CDuiString *>(m_SharedResInfo.m_AttrHash.Find(key));
                if (pDefaultAttr)
                    delete pDefaultAttr;
            }
        }
...
LPCTSTR CPaintManagerUI::GetWindowCustomAttribute(LPCTSTR pstrName) const
{
    ...
    CDuiString *pCostomAttr =
        static_cast<CDuiString *>(m_mWindowCustomAttrHash.Find(pstrName));
    if (pCostomAttr)
...
CControlUI *CPaintManagerUI::FindSubControlByPoint(CControlUI *pParent, POINT pt) const
{
    if (pParent == NULL)
        pParent = GetRoot();
    ASSERT(pParent);
    return pParent->FindControl(__FindControlFromPoint, &pt,
                                UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST);
}

CControlUI *CPaintManagerUI::FindSubControlByName(CControlUI *pParent,
...
CControlUI *CPaintManagerUI::FindSubControlByClass(CControlUI *pParent, LPCTSTR pstrClass,
                                                   int iIndex)
{
    if (pParent == NULL)
        pParent = GetRoot();
...
CStdPtrArray *CPaintManagerUI::FindSubControlsByClass(CControlUI *pParent,
                                                      LPCTSTR pstrClass)
{
    if (pParent == NULL)
        pParent = GetRoot();
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromNameHash(CControlUI *pThis,
                                                                LPVOID pData)
{
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromCount(CControlUI * /*pThis*/,
                                                             LPVOID pData)
{
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromPoint(CControlUI *pThis,
                                                             LPVOID pData)
{
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromTab(CControlUI *pThis,
                                                           LPVOID pData)
{
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromShortcut(CControlUI *pThis,
                                                                LPVOID pData)
{
    if (!pThis->IsVisible())
        return NULL;
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromName(CControlUI *pThis,
                                                            LPVOID pData)
{
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlFromClass(CControlUI *pThis,
                                                             LPVOID pData)
{
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlsFromClass(CControlUI *pThis,
                                                              LPVOID pData)
{
    LPCTSTR pstrType = static_cast<LPCTSTR>(pData);
    LPCTSTR pType = pThis->GetClass();
...
CControlUI *CALLBACK CPaintManagerUI::__FindControlsFromUpdate(CControlUI *pThis,
                                                               LPVOID pData)
{
    ...
    if (uChildRes != 0) {
        ...
        for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
            ...
            while (hTempParent) {
                if (pMsg->hwnd == pT->GetPaintWindow() ||
                    ...
                    pT->PreMessageHandler(pMsg->message, pMsg->wParam, pMsg->lParam,
                                          lRes);
                }
        ...
        for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
            ...
            if (pMsg->hwnd == pT->GetPaintWindow()) {
                ...
                if (pT->PreMessageHandler(pMsg->message, pMsg->wParam, pMsg->lParam,
                                          lRes))
                    return true;
...
bool CPaintManagerUI::AddTranslateAccelerator(
    ITranslateAccelerator *pTranslateAccelerator)
{
...
bool CPaintManagerUI::OnDrop(FORMATETC *pFmtEtc, STGMEDIUM &medium, DWORD *pdwEffect)
{
    ...
    if (pFmtEtc->cfFormat == CF_DIB && medium.tymed == TYMED_HGLOBAL) {
        if (medium.hGlobal != NULL) {
            ...
            if (lpbi != NULL) {
                ...
                if (hbm != NULL)
                    hbm = (HBITMAP) SendMessage(m_hTargetWnd, STM_SETIMAGE, IMAGE_BITMAP,
                                                (LPARAM) hbm);
                if (hbm != NULL)
                    DeleteObject(hbm);
    ...
    if (pFmtEtc->cfFormat == CF_BITMAP && medium.tymed == TYMED_GDI) {
        if (medium.hBitmap != NULL) {
            HBITMAP hBmp = (HBITMAP) SendMessage(m_hTargetWnd, STM_SETIMAGE, IMAGE_BITMAP,
                                                 (LPARAM) medium.hBitmap);
            if (hBmp != NULL)
                DeleteObject(hBmp);
    ...
    if (pFmtEtc->cfFormat == CF_ENHMETAFILE && medium.tymed == TYMED_ENHMF) {
        ...
        HGDIOBJ hBmpMem =
            CreateCompatibleBitmap(hDC, emh.rclBounds.right, emh.rclBounds.bottom);
        ...
        hBmp = (HBITMAP) SendMessage(m_hTargetWnd, STM_SETIMAGE, IMAGE_BITMAP,
                                     (LPARAM) hBmp);
        if (hBmp != NULL)
            DeleteObject(hBmp);
        return true;
    ...
    if (pFmtEtc->cfFormat == CF_HDROP && medium.tymed == TYMED_HGLOBAL) {
        ...
        if (hDrop != NULL) {
            ...
            if (cFiles > 0) {
                ...
                if (hBitmap) {
                    HBITMAP hBmp = (HBITMAP) SendMessage(m_hTargetWnd, STM_SETIMAGE,
