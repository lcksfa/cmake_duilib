...
void CChildLayoutUI::Init()
{
    if (!m_pstrXMLFile.IsEmpty()) {
        ...
        CContainerUI *pChildWindow = static_cast<CContainerUI *>(
            builder.Create(m_pstrXMLFile.GetData(), (UINT) 0, NULL, m_pManager));
        if (pChildWindow) {
...
void CChildLayoutUI::SetChildLayoutXML(ui::CDuiString pXML)
{
    m_pstrXMLFile = pXML;
}

ui::CDuiString CChildLayoutUI::GetChildLayoutXML()
{
