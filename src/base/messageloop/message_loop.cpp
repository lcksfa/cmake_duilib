...
void MessageLoop::AddToIncomingQueue(const PendingTask &task)
{
    ...
    std::shared_ptr<IMessagePump> pump;
    {
...
bool MessageLoop::DoDelayedWork(ckbase::TimeTicks *next_delayed_work_time)
{
...
MessageLoop::PendingTask::PendingTask(const StdClosure &task, TimeTicks delayed_run_time,
                                      bool nestable)
    : std_task(task)
