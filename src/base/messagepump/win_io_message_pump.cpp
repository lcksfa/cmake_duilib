...
void WinIOMessagePump::ScheduleWork()
{
    ...
    // Make sure the IMessagePump does some work for us.
    BOOL ret = ::PostQueuedCompletionStatus(port_, 0, reinterpret_cast<ULONG_PTR>(this),
...
bool WinIOMessagePump::WaitForIOCompletion(DWORD timeout, IOHandler *filter)
{
    ...
    if (item.context->handler) {
        if (filter && item.handler != filter) {
            ...
            PreProcessIOEvent();
            item.handler->OnIOCompleted(item.context, item.bytes_transferred, item.error);
            PostProcessIOEvent();
        }
    }
    else {
...
bool WinIOMessagePump::GetIOItem(DWORD timeout, IOItem *item)
{
    ...
    if (!::GetQueuedCompletionStatus(port_.Get(), &item->bytes_transferred, &key,
                                     &overlapped, timeout)) {
        if (!overlapped)
