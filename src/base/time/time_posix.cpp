...
Time Time::FromTimeStruct(bool is_local, TimeStruct &ts)
{
    ...
    if (seconds == -1 && (ts.year() < 1969 || ts.year() > 1970)) {
        ...
        if (ts.year() < 1969) {
            ...
            milliseconds = (std::numeric_limits<time_t>::max() * kMillisecondsPerSecond) +
...
TimeTicks TimeTicks::Now()
{
    ...
    absolute_micro = (static_cast<int64>(ts.tv_sec) * Time::kMicrosecondsPerSecond) +
...
TimeTicks TimeTicks::Now()
{
    ...
    absolute_micro = (static_cast<int64_t>(tv.tv_sec) * Time::kMicrosecondsPerSecond) +
